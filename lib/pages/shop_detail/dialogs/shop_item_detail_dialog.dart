    ...
    return Dialog(
      ...
      child: Consumer2<UserProvider, AdsProvider>(
        builder: (context, provider, adsProvider, child) {
          ...
          bool hasAvatarAchv =
              user?.achievements!.contains('ACHV_003') ?? false;
          bool hasBackgroundAchv =
              user?.achievements!.contains('ACHV_004') ?? false;
          bool hasBorderAchv =
              user?.achievements!.contains('ACHV_005') ?? false;
          bool hasBannerAchv =
              user?.achievements!.contains('ACHV_006') ?? false;
          return Column(
            ...
            children: [
              ...
              ShopDialogBackgroundImageWidget(
                category: widget.category,
                imageUrl: widget.item.localPath ?? "${widget.item.imageUrl}",
              ),
              ...
              CcOutlinedButton(
                color: (itemPrice == 1500) ? premiumColor : primaryColor,
                onTap: () async {
                    ...
                    if (itemPrice == -1) {
                      if (AdsService.instance.isReady(
                        ...
                        AdsService.instance.show(
                          ...
                          () async {
                            int itemProgress =
                                adsProvider.adsBorderProgressCount;
                            itemProgress++;
                            adsProvider.setAdsBorderProgressCount =
                                itemProgress;
                            if (itemProgress == 5) {
                              await provider.updateUserDataForBuyItem(
                                widget.item.id ?? '',
                              );
                            }
                          },
                      ...
                      Utils.showWelcomToast(context, "Buy Premium!");
                    } else {
                ...
                radius: 8,
                child:
                    (widget.ownList.contains(widget.item.id))
                        ...
                        ? Row(
                          ...
                          children: [
                            ...
                            CcShadowedTextWidget(text: CcConstants.kPremium),
                          ],
                        )
                        : (itemPrice == -1)
                        ? CcShadowedTextWidget(
                          text:
                              (adsProvider.adsBorderProgressCount < 5)
