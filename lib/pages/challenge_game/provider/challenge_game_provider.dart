...
class ChallengeGameProvider extends ChangeNotifier {
  ChallengeGameProvider(
      ...
      if (_pageController!.hasClients) {
        ...
        if (answerId == guessId) {
          if (_currentComplete < _guessList.length) {
            _pageController!.animateToPage(_currentComplete,
                duration: const Duration(milliseconds: 300),
                curve: Curves.easeInOut);
          } else {
            Navigator.of(context)
                .pushReplacementNamed(RoutePaths.challengeVictory, arguments: [
              _mode,
              _remainingLife,
              _currentComplete,
              isReplay(chList)
  ...
  bool isReplay(List<ChallengeCompletedModel> list) {
    final int index = list.indexWhere((item) => item.mode == _mode);
    if (index != -1) {
      int temp = int.parse(list[index].complete ?? '0');
      if (temp < guessList.length) {
        return false;
      }
    }
    return true;
  ...
  void hint(
      BuildContext context, int coin, List<ChallengeCompletedModel> chList) {
    if (coin >= 10) {
