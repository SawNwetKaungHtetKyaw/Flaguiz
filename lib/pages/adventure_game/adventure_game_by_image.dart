import 'package:flaguiz/models/guess_model.dart';
import 'package:flaguiz/pages/adventure_game/provider/adventure_game_provider.dart';
import 'package:flaguiz/pages/adventure_game/widgets/adventure_game_answer_widget.dart';
import 'package:flaguiz/pages/adventure_game/widgets/adventure_game_guess_box_widget.dart';
import 'package:flaguiz/pages/adventure_game/widgets/adventure_game_hint_widget.dart';
import 'package:flaguiz/pages/adventure_game/widgets/adventure_game_life_widget.dart';
import 'package:flaguiz/pages/adventure_game/widgets/adventure_game_you_lose_widget.dart';
import 'package:flaguiz/providers/user_provider.dart';
import 'package:flaguiz/service/audio_service.dart';
import 'package:flaguiz/utils/asset_images.dart';
import 'package:flaguiz/widgets/cc_back_widget.dart';
import 'package:flaguiz/widgets/cc_shadowed_icon_widget.dart';
import 'package:flaguiz/widgets/cc_shadowed_text_widget.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class AdventureGameByImage extends StatefulWidget {
  const AdventureGameByImage(
      {super.key,
      required this.guessList,
      required this.mode,
      required this.levelId});
  final List<GuessModel> guessList;
  final String mode;
  final String levelId;

  @override
  State<AdventureGameByImage> createState() => _AdventureGameByImageState();
}

class _AdventureGameByImageState extends State<AdventureGameByImage> {
  final PageController _pageController = PageController();

  @override
  void initState() {
    super.initState();
    AudioService.instance.pause();
  }

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<AdventureGameProvider>(
      create: (context) => AdventureGameProvider(
          buildContext: context,
          pageController: _pageController,
          guessList: widget.guessList,
          mode: widget.mode,
          levelId: widget.levelId),
      builder: (context, child) =>
          Consumer2<AdventureGameProvider, UserProvider>(
        builder: (context, provider, userProvider, child) => Scaffold(
          body: Container(
            width: double.maxFinite,
            height: double.maxFinite,
            decoration: const BoxDecoration(
                image: DecorationImage(
                    image: AssetImage(AssetsImages.adventureBg),
                    fit: BoxFit.cover)),
            child: Stack(
              children: [
                SafeArea(
                    child: Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: Column(
                    children: [
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          CcBackWidget(
                            image: AssetsImages.adventureBackKey,
                            onTap: () {
                              Navigator.pop(context);
                              AudioService.instance.resume();
                            },
                          ),

                          /// Adventure Life Widget
                          const AdventureGameLifeWidget()
                        ],
                      ),

                      /// Timer
                      Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          const CcShadowedIconWidget(
                              color: Colors.white,
                              icon: Icons.timer_outlined,
                              size: 40),
                          Container(
                            width: 30,
                            alignment: Alignment.center,
                            child: CcShadowedTextWidget(
                              text: provider.timerCount.toString(),
                              fontSize: 14,
                              textColor: (provider.timerCount) <= 3
                                  ? Colors.red
                                  : Colors.white,
                            ),
                          )
                        ],
                      ),

                      Expanded(
                          child: PageView.builder(
                              controller: _pageController,
                              physics: const NeverScrollableScrollPhysics(),
                              itemCount: provider.guessList.length,
                              itemBuilder: (context, index) {
                                GuessModel guess = provider.guessList[index];

                                /// Correct Answer for Each Guess
                                provider.setCorrectAnswerIndex =
                                    guess.countryList?.indexWhere(
                                            (u) => u.id == guess.answer!.id) ??
                                        0;
                                return Column(
                                  crossAxisAlignment: CrossAxisAlignment.center,
                                  children: [
                                    /// Answer
                                    AdventureGameAnswerWidget(
                                        guess: guess, mode: provider.mode),

                                    /// Guess
                                    Row(
                                      mainAxisAlignment:
                                          MainAxisAlignment.spaceBetween,
                                      children: [
                                        AdventureGameGuessBoxWidget(
                                            country: guess.countryList![0],
                                            answerId: guess.answer?.id ?? "0",
                                            adventureCompletedList: userProvider
                                                    .user
                                                    ?.adventureCompletedList ??
                                                [],
                                            currentIndex: index,
                                            index: 0),
                                        AdventureGameGuessBoxWidget(
                                            country: guess.countryList![1],
                                            answerId: guess.answer?.id ?? "0",
                                            adventureCompletedList: userProvider
                                                    .user
                                                    ?.adventureCompletedList ??
                                                [],
                                            currentIndex: index,
                                            index: 1),
                                      ],
                                    ),
                                    Row(
                                      mainAxisAlignment:
                                          MainAxisAlignment.spaceBetween,
                                      children: [
                                        AdventureGameGuessBoxWidget(
                                            country: guess.countryList![2],
                                            answerId: guess.answer?.id ?? "0",
                                            adventureCompletedList: userProvider
                                                    .user
                                                    ?.adventureCompletedList ??
                                                [],
                                            currentIndex: index,
                                            index: 2),
                                        AdventureGameGuessBoxWidget(
                                            country: guess.countryList![3],
                                            answerId: guess.answer?.id ?? "0",
                                            adventureCompletedList: userProvider
                                                    .user
                                                    ?.adventureCompletedList ??
                                                [],
                                            currentIndex: index,
                                            index: 3),
                                      ],
                                    )
                                  ],
                                );
                              })),

                      /// Hint Button
                      AdventureGameHintWidget(
                          adventureCompletedList:
                              userProvider.user?.adventureCompletedList ?? [],
                          coin: userProvider.user?.coin ?? 0)
                    ],
                  ),
                )),

                /// This Widget will show when your remaining life is over
                AdventureGameYouLoseWidget(guessList: provider.guessList),
              ],
            ),
          ),
        ),
      ),
    );
  }
}
